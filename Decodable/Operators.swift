...
public func => <T: Decodable>(lhs: AnyObject, rhs: String) throws -> [T] {
    return try parse(lhs, path: rhs, decode: safeDecodeArray(T.decode))
}
...
public func => <T: Decodable>(lhs: AnyObject, rhs: String) throws -> [T]? {
    return try parse(lhs, path: rhs, decode: catchNull(safeDecodeArray(T.decode)))
}

/// Try to decode as NSArray, and decode each element as T or nil, if the element is NSNull.
public func => <T: Decodable>(lhs: AnyObject, rhs: String) throws -> [T?] {
    return try parse(lhs, path: rhs, decode: safeDecodeArray(catchNull(T.decode)))
}
